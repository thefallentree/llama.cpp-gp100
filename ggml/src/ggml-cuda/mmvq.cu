...
#elif defined(__CUDA_ARCH__) && !defined(GGML_USE_HIP) && !defined(GGML_USE_MUSA) && \
    __CUDA_ARCH__ == GGML_CUDA_CC_PASCAL
    return MMVQ_PARAMETERS_NVIDIA_SM60;
...
static __host__ mmvq_parameter_table_id get_device_table_id(int cc) {
    ...
    if (cc == GGML_CUDA_CC_PASCAL && ggml_cuda_highest_compiled_arch(cc) == GGML_CUDA_CC_PASCAL) {
        return MMVQ_PARAMETERS_NVIDIA_SM60;
    }
...
static constexpr __host__ __device__ int calc_nwarps(ggml_type type, int ncols_dst, mmvq_parameter_table_id table_id, bool small_k = false, bool halve_iters = false) {
    if (table_id == MMVQ_PARAMETERS_GENERIC || table_id == MMVQ_PARAMETERS_NVIDIA_SM60) {
        switch (ncols_dst) {
            case 1:
            case 2:
            case 3:
...
static constexpr __host__ __device__ int calc_rows_per_block(
    ...
    if (table_id == MMVQ_PARAMETERS_GENERIC || table_id == MMVQ_PARAMETERS_NVIDIA_SM60 ||
        ...
        switch (ncols_dst) {
            case 1:
                // Reuse each activation row across four output rows on SM60.
                if (table_id == MMVQ_PARAMETERS_NVIDIA_SM60) {
                    return 4;
                }
...
static __global__ void mul_mat_vec_q(
    ...
    if constexpr (has_fusion) {
        ...
        if (threadIdx.x < rows_per_cuda_block && threadIdx.y == 0 &&
            (!check_rows || rows_per_cuda_block == 1 || uint32_t(row0 + threadIdx.x) < nrows_x)) {
            if (use_bias) {
...
#pragma unroll
            for (int i = 0; i < rows_per_cuda_block; ++i) {
                if (!check_rows || rows_per_cuda_block == 1 || uint32_t(row0 + i) < nrows_x) {
...
#pragma unroll
            ...
            if (threadIdx.x == i &&
                    (!check_rows || rows_per_cuda_block == 1 || uint32_t(row0 + i) < nrows_x)) {
...
static void mul_mat_vec_q_switch_fusion(
        ...
        const uint32_t stride_channel_y, const uint32_t stride_channel_dst, const uint3 sample_ratio,
        const uint32_t stride_sample_x, const uint32_t stride_sample_y, const uint32_t stride_sample_dst,
        const bool check_rows, const dim3 & block_nums, const dim3 & block_dims, const int nbytes_shared,
        ...
        if constexpr (c_ncols_dst == 1) {
            if (has_fusion) {
                ggml_cuda_kernel_launch(
                    (mul_mat_vec_q<type, c_ncols_dst, true, small_k, halve_iters, false>), launch_params,
                    vx, vy, ids, fusion, dst, ncols_x, nrows_x, nchannels_y,
                    stride_row_x, stride_col_y, stride_col_dst,
                    channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
                    sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride);
            } else {
                ggml_cuda_kernel_launch(
                    (mul_mat_vec_q<type, c_ncols_dst, false, small_k, halve_iters, false>), launch_params,
                    vx, vy, ids, fusion, dst, ncols_x, nrows_x, nchannels_y,
                    stride_row_x, stride_col_y, stride_col_dst,
                    channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
                    sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride);
...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    const auto needs_row_bounds = [&](int width) {
        const int nwarps = calc_nwarps(type, width, table_id);
        const int rows_per_block = calc_rows_per_block(width, table_id, false, nwarps);
        return nrows_x % rows_per_block != 0;
    };
    ...
    switch (ncols_dst) {
        case 1: {
            ...
            const auto launch = [&](auto small_k_tag, auto halve_iters_tag) {
                ...
                const int rows_per_block =
                    calc_rows_per_block(c_ncols_dst, table_id, c_small_k,
                                        calc_nwarps(type, c_ncols_dst, table_id, c_small_k, c_halve_iters));
                const bool check_rows = nrows_x % rows_per_block != 0;
                ...
                mul_mat_vec_q_switch_fusion<type, c_ncols_dst, c_small_k, c_halve_iters>(
                    ...
                    stride_sample_x, stride_sample_y, stride_sample_dst, check_rows,
                    dims.first, dims.second, 0, ids_stride,
                    stream);
            };
        ...
        case 2: {
            ...
            mul_mat_vec_q_switch_fusion<type, c_ncols_dst>(
                 ...
                 needs_row_bounds(c_ncols_dst), dims.first, dims.second, 0, ids_stride, stream);
        } break;
        case 3: {
