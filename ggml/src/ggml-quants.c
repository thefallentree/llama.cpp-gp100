...
void quantize_row_q4_1_g64_ref(const float * GGML_RESTRICT x, block_q4_1_g64 * GGML_RESTRICT y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int h = 0; h < 2; ++h) {
            ...
            for (int j = 0; j < 16; ++j) {
                const uint8_t lo = MIN(15, (int8_t) ((xx[j     ] - min)*id + 0.5f));
                const uint8_t hi = MIN(15, (int8_t) ((xx[j + 16] - min)*id + 0.5f));
                qs[j] = lo | (hi << 4);
            }
        }
    }
...
static void quantize_row_q4_1_g64_impl(
        const float * GGML_RESTRICT x, block_q4_1_g64 * GGML_RESTRICT y,
