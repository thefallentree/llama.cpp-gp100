...
void dequantize_row_q4_1_g64(const block_q4_1_g64 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    static const int qk = QK4_1_G64;
    assert(k % qk == 0);
    const int nb = k / qk;
    for (int i = 0; i < nb; i++) {
        const float d = GGML_FP16_TO_FP32(x[i].d);
        const float m = GGML_FP16_TO_FP32(x[i].m);
        for (int h = 0; h < 2; ++h) {                 // two Q4_1-layout 16-byte halves
            const uint8_t * qs = x[i].qs + 16*h;
            float * yy = y + i*qk + 32*h;
            for (int j = 0; j < 16; ++j) {
                yy[j     ] = (qs[j] & 0x0F)*d + m;
                yy[j + 16] = (qs[j] >>   4)*d + m;
            }
...
void quantize_row_q4_1_g64_ref(const float * GGML_RESTRICT x, block_q4_1_g64 * GGML_RESTRICT y, int64_t k) {
    static const int qk = QK4_1_G64;
    assert(k % qk == 0);
    const int nb = k / qk;
    for (int i = 0; i < nb; i++) {
        ...
        y[i].d = GGML_FP32_TO_FP16(d);
        y[i].m = GGML_FP32_TO_FP16(vmin);
        for (int h = 0; h < 2; ++h) {
            uint8_t * qs = y[i].qs + 16*h;
            const float * xx = x + i*qk + 32*h;
            for (int j = 0; j < 16; ++j) {
                const uint8_t lo = (uint8_t) MIN(15, (int8_t) roundf((xx[j     ] - vmin)*id));
                const uint8_t hi = (uint8_t) MIN(15, (int8_t) roundf((xx[j + 16] - vmin)*id));
                qs[j] = lo | (hi << 4);
            }
        }
    }
...
static void quantize_row_q4_1_g64_impl(const float * GGML_RESTRICT x, block_q4_1_g64 * GGML_RESTRICT y, int64_t n_per_row, const float * quant_weights) {
    static_assert(QK4_1_G64 == 64, "QK4_1_G64 must be 64");

    if (!quant_weights) {
        quantize_row_q4_1_g64_ref(x, y, n_per_row);
        return;
    }

    float weight[QK4_1_G64];
    uint8_t L[QK4_1_G64], Laux[QK4_1_G64];

    float sum_x2 = 0;
    for (int j = 0; j < n_per_row; ++j) sum_x2 += x[j]*x[j];
    float sigma2 = sum_x2/n_per_row;

    const int64_t nb = n_per_row/QK4_1_G64;
    for (int ib = 0; ib < nb; ++ib) {
        const float * xb = x + QK4_1_G64 * ib;
        const float * qw = quant_weights + QK4_1_G64 * ib;
        for (int j = 0; j < QK4_1_G64; ++j) weight[j] = qw[j] * sqrtf(sigma2 + xb[j]*xb[j]);
        float min;
        float d = make_qkx3_quants(QK4_1_G64, 15, xb, weight, L, &min, Laux, -0.9f, 0.05f, 36, false);
        y[ib].d = GGML_FP32_TO_FP16(d);
        y[ib].m = GGML_FP32_TO_FP16(-min);
        // two 16-byte Q4_1-layout halves behind the shared (d, m)
        for (int h = 0; h < 2; ++h) {
            uint8_t * qs = y[ib].qs + 16*h;
            const uint8_t * Lh = L + 32*h;
            for (int j = 0; j < 16; ++j) {
                qs[j] = Lh[j] | (Lh[j + 16] << 4);
            }
        }
...
size_t quantize_q4_1_g64(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    if (!quant_weights) {
        quantize_row_q4_1_g64_ref(src, dst, (int64_t)nrow*n_per_row);
        return nrow * ggml_row_size(GGML_TYPE_Q4_1_G64, n_per_row);
    }
    size_t row_size = ggml_row_size(GGML_TYPE_Q4_1_G64, n_per_row);
    char * qrow = (char *)dst;
    for (int64_t row = 0; row < nrow; ++row) {
        quantize_row_q4_1_g64_impl(src, (block_q4_1_g64*)qrow, n_per_row, quant_weights);
        src += n_per_row;
        qrow += row_size;
    }
    return nrow * row_size;
...
bool ggml_validate_row_data(enum ggml_type type, const void * data, size_t nbytes) {
    ...
    switch (type) {
        ...
        case GGML_TYPE_Q4_1_G64:
            {
                VALIDATE_ROW_DATA_DM_F16_IMPL(block_q4_1_g64, data, nb, d, m);
            } break;
        case GGML_TYPE_Q5_0:
            {
