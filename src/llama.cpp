...
static bool llama_prepare_model_devices(const llama_model_params & params, llama_model * model) {
    ...
    if (params.devices) {
        if (params.split_mode == LLAMA_SPLIT_MODE_TENSOR) {
            ...
            if (n_devs == 1) {
                // Tensor parallelism over a single device splits nothing, but the
                // Meta wrapper still has to infer a split state for every op.  Use
                // the device directly so a one-device model (a draft model pinned
                // with --device-draft, say) is not constrained by that inference.
                LLAMA_LOG_INFO("%s: single device %s, skipping the Meta device\n",
                        __func__, ggml_backend_dev_name(params.devices[0]));
